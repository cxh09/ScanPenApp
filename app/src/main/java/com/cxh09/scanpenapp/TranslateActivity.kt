...
/**
 * 「翻译」技能运行页：与 [LivelyActivity] 同构，仅替换输入占位与 system prompt。
 */
class TranslateActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.etInput.hint = getString(R.string.skill_run_input_hint_translate)
            ...
            val outcome = runCatching {
                withContext(Dispatchers.IO) {
                    ...
                    val request = clientHolder.buildChatRequest(
                        config,
                        listOf(
                            systemMessage(getString(R.string.ai_skill_translate_prompt)),
                            userMessage(text),
                        ),
                    )
    ...
    private fun onAboutClicked() {
        SkillAboutActivity.start(this, SkillAboutActivity.KEY_TRANSLATE)
    }
        ...
        val detector = GestureDetector(this, object : GestureDetector.SimpleOnGestureListener() {
            override fun onDoubleTap(e: MotionEvent): Boolean {
                ...
                AiMessageFullscreenActivity.start(
                    ...
                    text,
                    getString(R.string.ai_skill_translate_name),
                    null,
                )
                return true
            }
